...
В решениях всех задач нельзя использовать циклы `for` и `while`, а также метод `forEach` массивов.
Вместо этого используйте методы массивов `map`, `filter`, `reduce`, `sort`, `every`, `some`

### Задача 1

Напишите функцию `sum`, которая суммирует все переданные в нее агрументы.
...
*Выходные данные:* массив из x, y координат марсохода
```javascript
{ x: -1, y: 5 }
```
...
Теперь объединим две предыдущие задачи.
Напишите функцию, которая принимает несколько серий передвижений марсохода, каждая из которых начинается в начале координат.
После каждой такой серии марсоход может оказаться в разных точках.
Напишите функцию которая определит минимальный ограничивающий прямоугольник для этих возможных точек.
Ваш код должен импортировать и использовать уже готовые решения из задач 3 и 4.
